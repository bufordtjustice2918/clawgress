...
DEFAULT_MVP_FULL_COMMANDS = [
    ...
    "show configuration commands | match \"service clawgress policy proxy backend\" | no-more | grep -q none",
    ...
    "show clawgress telemetry agents | no-more",
    "show clawgress telemetry domains | no-more",
    "show clawgress telemetry denies | no-more",
    "sudo /usr/bin/clawgress telemetry export --window 1h | no-more | grep -q '\"telemetry_schema_version\": 2'",
    "sudo /usr/bin/clawgress telemetry export --window 1h | no-more | grep -q '<redacted>'",
    "sudo /usr/bin/clawgress telemetry export --window 1h --no-redact | no-more | grep -q '\"source_ips\": {'",
    "! sudo /usr/bin/clawgress telemetry export --window 1h --no-redact | no-more | grep -q '<redacted>'",
    "curl -sS --unix-socket /run/api.sock -H 'Content-Type: application/json' -d '{\"key\":\"id_key\",\"view\":\"export\",\"window\":\"1h\",\"redact\":true}' http://localhost/clawgress/telemetry | grep -q '\"success\": true'",
    "curl -sS --unix-socket /run/api.sock -H 'Content-Type: application/json' -d '{\"key\":\"id_key\",\"view\":\"export\",\"window\":\"1h\",\"redact\":true}' http://localhost/clawgress/telemetry | grep -q '<redacted>'",
    "curl -sS --unix-socket /run/api.sock -H 'Content-Type: application/json' -d '{\"key\":\"id_key\",\"view\":\"export\",\"window\":\"1h\",\"redact\":false}' http://localhost/clawgress/telemetry | grep -q '\"success\": true'",
    "curl -sS --unix-socket /run/api.sock -H 'Content-Type: application/json' -d '{\"key\":\"id_key\",\"view\":\"export\",\"window\":\"1h\",\"redact\":false}' http://localhost/clawgress/telemetry | grep -q '\"source_ips\": {'",
    "! curl -sS --unix-socket /run/api.sock -H 'Content-Type: application/json' -d '{\"key\":\"id_key\",\"view\":\"export\",\"window\":\"1h\",\"redact\":false}' http://localhost/clawgress/telemetry | grep -q '<redacted>'",
    "curl -sS --unix-socket /run/api.sock -H 'Content-Type: application/json' -d '{\"key\":\"id_key\",\"policy\":{\"version\":1,\"allow\":{\"domains\":[\"api.openai.com\"],\"ips\":[],\"ports\":[53,443]},\"proxy\":{\"mode\":\"sni-allowlist\",\"backend\":\"nginx\",\"domains\":[\"api.openai.com\"]},\"hosts\":{}},\"apply\":false}' http://localhost/clawgress/policy | grep -q '\"success\": false'",
    "curl -sS --unix-socket /run/api.sock -H 'Content-Type: application/json' -d '{\"key\":\"id_key\",\"policy\":{\"version\":1,\"allow\":{\"domains\":[\"api.openai.com\"],\"ips\":[],\"ports\":[53,443]},\"proxy\":{\"mode\":\"sni-allowlist\",\"backend\":\"nginx\",\"domains\":[\"api.openai.com\"]},\"hosts\":{}},\"apply\":false}' http://localhost/clawgress/policy | grep -q 'policy.proxy.backend'",
    "RAW: configure",
    "RAW: set service clawgress policy proxy backend haproxy",
    "RAW: commit",
    "RAW: save",
    "RAW: run show configuration commands | match \"service clawgress policy proxy backend\" | no-more | grep -q haproxy",
    "RAW: run show clawgress status | no-more | grep -q '\"haproxy_active\": true'",
    "RAW: exit discard",
